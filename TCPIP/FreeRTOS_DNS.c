 ...
 */
static const uint8_t *prvSkipNameField( uint16_t *pBytesLeft, const uint8_t *pucByte );

/*
 * Process a response packet from a DNS server.
 */
static uint32_t prvParseDNSReply( uint16_t xUDPPayloadSize, uint8_t *pucUDPPayloadBuffer, TickType_t xIdentifier );
 ...
 */
	...
	static const uint8_t *prvReadNameField( uint16_t *pBytesLeft, const uint8_t *pucByte, char *pcName, BaseType_t xLen );
			...
			if( pucUDPPayloadBuffer != NULL )
			{
				...
				if( FreeRTOS_sendto( xDNSSocket, pucUDPPayloadBuffer, xPayloadLength, FREERTOS_ZERO_COPY, &xAddress, sizeof( xAddress ) ) != 0 )
				{
					...
					if( lBytes > 0 )
					{
						...
						ulIPAddress = prvParseDNSReply( lBytes, pucUDPPayloadBuffer, xIdentifier );
...
/*-----------------------------------------------------------*/
static portINLINE const uint8_t *CheckNameFieldValid(uint16_t bytesLeft, const uint8_t *pBuffer)        
{
    if( !pBuffer )
    {
        return NULL;
    ...
    // This assumes there's always something afterwards - another name field or something
    const uint8_t *pResult = pBuffer;
    
    if(bytesLeft > 1)
    {
        pResult += *pBuffer + 1;
        
        if(bytesLeft < (pResult - pBuffer + 1))
        {
            return NULL;
        }
    }
...
static portINLINE const uint8_t *CheckAndIncrementPtr(uint16_t *pBytesLeft, uint16_t bytesToInc, uint16_t bytesAfterInc, const uint8_t *pBuffer)
{
    if((bytesToInc + bytesAfterInc) > *pBytesLeft)
    {
        return NULL;
    }
    
    *pBytesLeft -= bytesToInc;
    
    return pBuffer + bytesToInc;
}
	...
	{        
                if(*pBytesLeft > 0)
                {
                        ...
                        if( ( *pucByte & dnsNAME_IS_OFFSET ) == dnsNAME_IS_OFFSET )
                        {
                                ...
                                if(*pBytesLeft >= sizeof(uint16_t))
                                {
                                    pucByte += sizeof( uint16_t );
                                    *pBytesLeft -= sizeof(uint16_t);
                                }
                                else
                                {
                                    pucByte = NULL;
                        ...
                        else
                        {
                                ...
                                while( *pucByte != 0x00 )
                                {
                                        const uint8_t *pNextField = CheckNameFieldValid(*pBytesLeft, pucByte);
...
DNSMessage_t *pxDNSMessageHeader = ( DNSMessage_t * ) pucUDPPayloadBuffer;
	...
	prvParseDNSReply( pxNetworkBuffer->xDataLength, pucUDPPayloadBuffer, ( uint32_t ) pxDNSMessageHeader->usIdentifier );
...
static uint32_t prvParseDNSReply( uint16_t xUDPPayloadSize, uint8_t *pucUDPPayloadBuffer, TickType_t xIdentifier )
{
	...
	if( ( xUDPPayloadSize > sizeof(DNSMessage_t) ) && ( pxDNSMessageHeader->usIdentifier == ( uint16_t ) xIdentifier ) )
	{
		/* Start at the first byte after the header. */
		pucByte = pucUDPPayloadBuffer + sizeof( DNSMessage_t );
                usDataLength = xUDPPayloadSize - sizeof( DNSMessage_t );

		/* Skip any question records. */
		usQuestions = FreeRTOS_ntohs( pxDNSMessageHeader->usQuestions );
		for( x = 0; pucByte && (x < usQuestions); x++ )
		{
#if( ipconfigUSE_LLMNR == 1 )
                        if( x == 0 )
                        {
                            const uint8_t *pNextNameField = CheckNameFieldValid(usDataLength, pucByte);

                            if( pNextNameField && (*pNextNameField == 0) )
                            {
			...
			if( x == 0 )
			{
				pucByte = prvReadNameField( &usDataLength, pucByte, pcName, sizeof( pcName ) );
			}
			else
#endif /* ipconfigUSE_DNS_CACHE */
			{
				/* Skip the variable length pcName field. */
				pucByte = prvSkipNameField( &usDataLength, pucByte );
			}

                        if( pucByte && (usDataLength >= sizeof(uint32_t)) )
                        {
                            #if( ipconfigUSE_LLMNR == 1 )
                            {
		...
		if( ( pxDNSMessageHeader->usFlags & dnsRX_FLAGS_MASK ) == dnsEXPECTED_RX_FLAGS )
		{
			for( x = 0; x < pxDNSMessageHeader->usAnswers; x++ )
			{
				...
				if( usChar2u16( pucByte ) == dnsTYPE_A_HOST )
				{
					...
					length. */
                                        pucByte = CheckAndIncrementPtr(&usDataLength, sizeof(uint32_t) + sizeof(uint32_t) + sizeof(uint8_t), sizeof(uint8_t), pucByte);

					/* Sanity check the data length. */
					if( pucByte && (( size_t ) *pucByte == sizeof( uint32_t ))  )
					{
						/* Skip the second byte of the length. */
                                                pucByte = CheckAndIncrementPtr(&usDataLength, sizeof(uint8_t), sizeof(uint32_t), pucByte);
				...
				else
				{
					...
					/* Skip the type, class and time to live fields. */
                                        pucByte = CheckAndIncrementPtr(&usDataLength, sizeof(uint32_t) + sizeof(uint32_t), sizeof(fieldLength), pucByte);
                                        ...
                                        memcpy(&fieldLength, pucByte, sizeof(fieldLength));
                                        fieldLength = FreeRTOS_ntohs(fieldLength);

                                        /* Jump over the data length bytes, and the data itself. */
                                        pucByte = CheckAndIncrementPtr(&usDataLength, fieldLength + sizeof(uint16_t), 0, pucByte);
				}
			}
		}
#if( ipconfigUSE_LLMNR == 1 )
		else if( usQuestions && ( usType == dnsTYPE_A_HOST ) && ( usClass == dnsCLASS_IN ) && pcRequestedName )
		{
