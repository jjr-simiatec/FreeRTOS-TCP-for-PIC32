 ...
 *
 * Note: Modified by John Robertson to add FAT12 formatting support.
 * 
 *******************************************************************************
...
FF_IOManager_t *pxIOManager = pxDisk->pxIOManager;
	...
	{
                if(ulSectorCount < MIN_CLUSTERS_FAT16)
                {
                    ucFATType = FF_T_FAT12;
                    iFAT16RootSectors = 14; /* to get 224 dir entries */
                }
                else
                {
	...
	for( ;; )
	{
		...
		ulUsableDataSectors = ulSectorCount - ulNonDataSectors;
                ...
                if((ucFATType == FF_T_FAT12) && (ulUsableDataClusters < MIN_CLUSTERS_FAT16))
                {
                        break;
                }
...
/*  ======================================================================================= */
	...
	FF_putLong (pucSectorBuffer, OFS_BPB_HiddSec_32, ( uint32_t ) ulHiddenSectors ); /* 0x01C / n.a.	0 for nonparitioned volume */
	{
		...
		FF_PRINTF("FF_Format: SecCluster %u DatSec %u DataClus %u ulClusterBeginLBA %u\r\n",
		...
		else
		{
			...
			memcpy( pucSectorBuffer + OFS_BPB_16_BS_VolLab_88, "MY NAME    ", 11 );            /* 0x02B / "NO NAME    " */
                        ...
                        if(ucFATType == FF_T_FAT16)
                        {
                            memcpy( pucSectorBuffer + OFS_BPB_16_FilSysType_64, "FAT16   ", 8 );           /* 0x036 / "FAT12   " */
                        }
                        else
                        {
                            memcpy( pucSectorBuffer + OFS_BPB_16_FilSysType_64, "FAT12   ", 8 );           /* 0x036 / "FAT12   " */
                        }
		}
		...
		switch( ucFATType )
		{
				...
				FF_putShort( pucSectorBuffer, 2, 0x00FF ); /* RESERVED alloc. */                            
                                break;
		...
		int32_t lLastAddress;
			...
			FF_PRINTF("FF_Format: Clearing root directory at %08X: %u sectors\r\n", dirBegin, lLastAddress - dirBegin );
