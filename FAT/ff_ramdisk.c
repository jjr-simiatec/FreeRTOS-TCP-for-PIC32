...
// Modified: February 2017 John Robertson to add mounting of preprepared disk images
...
*/
FF_Disk_t *RAMDiskCommonInit(uint8_t *pucDataBuffer, uint32_t ulSectorCount, bool bReadOnly, size_t xIOManagerCacheSize)
{
    ...
    FF_Disk_t *pxDisk = pvPortMalloc( sizeof(*pxDisk) );
    
    if( !pxDisk )
    {
        FF_PRINTF("RAMDiskCommonInit: malloc failed\n");
        return NULL;
    }
    ...
    if( !pxDisk->pxIOManager || FF_isERR(xError) )
    {
...
bool RAMDiskMount(FF_Disk_t *pxDisk, const char pcName[], BaseType_t xPartition)
{
    pxDisk->xStatus.bPartitionNumber = xPartition;
    
    /* Mount the partition. */
    FF_Error_t xError = FF_Mount(pxDisk, xPartition);
    
    if( FF_isERR(xError) )
    {
        FF_PRINTF("RAMDiskMount: FF_Mount: %s\n", (const char *) FF_GetErrMessage(xError));
        return false;
    }
    ...
    return FF_FS_Add(pcName, pxDisk) != 0;
}

FF_Disk_t *FF_RAMDiskInit( char *pcName, uint8_t *pucDataBuffer, uint32_t ulSectorCount, size_t xIOManagerCacheSize )
{
    FF_Disk_t *pxDisk = RAMDiskCommonInit(pucDataBuffer, ulSectorCount, false, xIOManagerCacheSize);
    ...
    FF_Error_t xError = prvPartitionAndFormatDisk(pxDisk);

    if( FF_isERR(xError) || !RAMDiskMount(pxDisk, pcName, ramPARTITION_NUMBER) )
    {
        FF_RAMDiskDelete(pxDisk);
        pxDisk = NULL;
    }
...
FF_Disk_t *FF_DiskImageReadOnlyInit(const char *pcName, const uint8_t pucDataBuffer[], size_t ulSectorCount, size_t xIOManagerCacheSize)
{
    FF_Disk_t *pxDisk = RAMDiskCommonInit((uint8_t *) pucDataBuffer, ulSectorCount, true, xIOManagerCacheSize);
    
    if( !pxDisk || !RAMDiskMount(pxDisk, pcName, ramPARTITION_NUMBER) )
    {
        FF_RAMDiskDelete(pxDisk);
        pxDisk = NULL;
    }
...
FF_Disk_t *FF_DiskImageInit(const char *pcName, uint8_t pucDataBuffer[], size_t ulSectorCount, size_t xIOManagerCacheSize)
{
    FF_Disk_t *pxDisk = RAMDiskCommonInit(pucDataBuffer, ulSectorCount, false, xIOManagerCacheSize);
